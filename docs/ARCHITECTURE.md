...
| Who may do what | two roles: admin and operator | An operator installs, reinstalls, calls an installation off and takes a blade out of service; everything that decides what the fleet *is* belongs to an admin. The table of actions is the boundary, and an action nobody has classified needs an admin |
